...
import com.example.lv5.ui.screen.PoiListScreen
...
fun LV5App(poiViewModel: PoiViewModel) {
    ...
    Scaffold(
        floatingActionButton = {
            FloatingActionButton(onClick = { navController.navigate(Screens.AddPoi.name) }) {
        ...
        Surface(modifier = Modifier.padding(innerPadding)) {
            NavHost(navController = navController, startDestination = Screens.PoiList.name) {
                composable(Screens.PoiList.name) {
                    ...
                    PoiListScreen(list = poiList.value, navigateToViewPoi = {
